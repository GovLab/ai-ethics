...
// Function to clean Directus asset URLs
function cleanDirectusUrl(url) {
  // Remove query parameters
  const cleanUrl = url.split('?')[0];
  
  // If it's a Directus asset URL, try different variations
  if (cleanUrl.includes('directus.thegovlab.com/ai-ethics/assets/')) {
    const assetId = cleanUrl.split('/').pop();
    return [
      `https://directus.thegovlab.com/ai-ethics/assets/${assetId}`,
      `https://directus.thegovlab.com/uploads/ai-ethics/originals/${assetId}`,
      `https://directus.thegovlab.com/ai-ethics/assets/${assetId}?key=thumbnail`,
      `https://directus.thegovlab.com/ai-ethics/assets/${assetId}?key=directus-medium-crop`
    ];
  }
...
async function tryDownloadImage(filename, urls, imagePath) {
  for (const url of urls) {
    try {
      ...
      console.log(`✗ Failed ${filename} from ${url}: ${err.message}`);
    }
  }
...
async function processImageReferences(obj, processedUrls = new Set()) {
  ...
  if (Array.isArray(obj)) {
    ...
    for (const item of obj) {
      results.push(await processImageReferences(item, processedUrls));
    }
    return results;
  }
    ...
    if (typeof value === 'string' && value.includes('directus.thegovlab.com')) {
      ...
      if (!processedUrls.has(value)) {
        ...
        if (!fs.existsSync(imagePath)) {
          ...
          const urlVariations = cleanDirectusUrl(value);
          await tryDownloadImage(filename, urlVariations, imagePath);
      ...
      result[key] = await processImageReferences(value, processedUrls);
    } else {
      result[key] = value;
    }
  }
...
async function processAllJsonFiles() {
  ...
  console.log('Processing JSON files for image references (v2)...\n');
  ...
  for (const jsonFile of jsonFiles) {
    ...
    const processedData = await processImageReferences(data);
